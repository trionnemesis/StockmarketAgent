...
**[開啟 GitHub Pages](https://trionnemesis.github.io/StockmarketAgent/)** · [Universe 證據審查](https://trionnemesis.github.io/StockmarketAgent/universe-review.html) · [來源可行性](https://trionnemesis.github.io/StockmarketAgent/source-feasibility.html) · [研究方法](https://trionnemesis.github.io/StockmarketAgent/methodology.html) · [資料狀態](https://trionnemesis.github.io/StockmarketAgent/status.html) · [驗證紀錄](VERIFICATION.md)
...
目前完整分析路徑使用 **deterministic synthetic research snapshot**。研究頁可出現 `BUY`、`HOLD`、`SELL`、`NO_SIGNAL`，但這些態度尚未完成 point-in-time（PIT）回測與校準，不能解讀為正式投資訊號。Risk Gate、Universe approval 與 production gate 仍然有效。

> **研究邊界：這是未校準的合成資料研究輸出，不是即時行情、個人化投資建議或交易指令。**
...
| Evidence review | 15 個 ETF tracking index、36 筆來源紀錄、6 組重疊與 issuer concentration 可供 Pages 閱讀 | reference metadata 不等於可用行情、PIT 歷史或再發布授權 |
...
相同 fixture、設定與程式版本會產生相同核心輸出。每個新研究 run 的 `git_sha` 指向最近一次修改 `config/`、`schemas/`、`src/` 或 `tests/fixtures/` 的完整 Git commit；`input_hash` 再綁定實際輸入與 build fingerprint。Pipeline 先在 staging 完整產生檔案並執行 preflight，再以 rollback-aware promotion 更新輸出；GitHub Pages workflow 只有在 build 與測試通過後才上傳完整 artifact。

## 快速開始

需求：Python 3.11+。研究 fixture build 不需安裝第三方套件，也不需 API key 或資料供應商帳號。
...
| `/instruments/<slug>.html` | 30 個候選標的的多期間態度、研究元件、風險與來源 |
| `/universe-review.html` | 逐檔 listing、ETF index、歷史、流動性、重疊與模型適用性審查 |
| `/source-feasibility.html` | 36 筆來源紀錄與採用、條件式使用、阻擋理由 |
...
- 目前輸入是固定、可重現的 synthetic research fixture，不是台灣、日本或美國市場的即時行情、財報、新聞或 ETF holdings。
...
- Repository 未啟用 live adapters、外部 provider credentials 或 provider data，也不宣稱已取得資料展示／再發布權。
...
| Source records | 36 | 不等同已整合 provider、取得 credentials 或再發布權 |
...
| Live adapters and provider data | 未啟用 | Repository 沒有 live credentials、行情或外部 provider dataset |
| PIT backtest and calibration | 未完成 | 研究態度不得視為已驗證的預測能力 |
| Production signals and trading | 未啟用 | 沒有正式 BUY／SELL、券商 API 或自動下單 |

正式資料功能必須以獨立、可審查的 PR 推進。下一階段至少要先完成 Owner 決策、external-display／redistribution rights、獲授權 PIT dataset、回測與校準；在這些 gate 通過前，系統維持 `research_only`。
